---
format: 
  revealjs:
    slide-number: true
    theme: [default, custom_theme.scss]
    preview-links: auto
    output-file: "index"
    output-ext: "html"
    output-dir: "docs"
    logo: "Materials/About/personal_sticker.png"
    code-line-numbers: true
    code-overflow: wrap
    embed-resources: true
    center-title-slide: false
---

```{r}
#| label: setup
#| include: false
options(htmltools.dir.version = FALSE)
knitr::opts_chunk$set(
  fig.align = "center",
  out.width = "100%",
  dpi = 300,
  fig.align = "center"
)


if (!require("countdown")) install.packages("countdown")

library(countdown)

library(here)

include_local_figure <- function(data_source) {
  knitr::include_graphics(
    path = here::here(
      "Materials",
      data_source
    ),
    error = TRUE
  )
}
```

# 

```{r}
#| label: lecture_intro
include_local_figure("flyer.png")
```

# 

:::: {.columns}

::: {.column width="60%"}
:::

::: {.column width="40%"}

```{r}
#| label: figure1
include_local_figure("flyer_cut.png")
```
:::

::::

::: {.r-hstack}
::: {data-id="box1" style="background: #fcf2dcff; width: 120px; height: 15px; margin: 0px;"}
:::

::: {data-id="box2" style="background: #c49cf5ff; width: 550px; height: 15px; margin: 0px;"}
:::

::: {data-id="box3" style="background: #040303ff; width: 120px; height: 15px; margin: 0px;"}
:::

::: {data-id="box3" style="background: #c2ff76ff; width: 120px; height: 15px; margin: 0px;"}
:::
:::

::: {.text-bold}
[Introduction to]{.text-font-lobster} [GitHub]{.text-background-wisteria .text-bold} [and]{.text-font-lobster} [Version Control]{.text-background-green-yellow .text-bold} 
:::

<br>

::: {.text-right}
Ondřej Mottl[👤](https://ondrejmottl.github.io/)

24.04.2025

Mendel University
:::

## 🙋[Hi!]{.text-background-purple} I am [Ondřej Mottl]{.text-background-orange}

<br>

::: {.text-center}
Assistant Professor at [Charles University](https://cuni.cz/UK-1.html)

Head of 🧑‍💻[Laboratory of Quantitative Ecology](https://ondrejmottl.github.io/lab/about_the_lab.html)

:::

<br>

:::: {.columns}

::: {.column width="60%"}
<br>

* 📧 Ondrej.mottl(at)gmail.com
* 🦋 [ondrejmottl.bsky.social](https://bsky.app/profile/ondrejmottl.bsky.social)
* 🐱 [Github OndrejMottl](https://github.com/OndrejMottl)
* 🆔 [ORCID 0000-0002-9796-5081](https://orcid.org/0000-0002-9796-5081)
* 🌐 [bit.ly/ondrej_mottl](https://bit.ly/ondrej_mottl)
:::

::: {.column width="35%"}
```{r}
#| label: head photo
include_local_figure("About/head_photo.png")
```
:::

::::

## This presentation {.text-center .bg-black .text-color-latte}

<br>
<br>

[😸 Code on GitHub](https://github.com/OndrejMottl/VersionControl_Mendel_Apr2025)

[![](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

<br>

:::: {.columns}

::: {.column width="33%"}
:::

::: {.column width="30%"}
```{r}
#| label: qr_slides
include_local_figure("QR_codes/qr_slides.png")
```
:::

::: {.column width="33%"}
:::

::::

## 

:::: {.columns}

::: {.column width="70%"}

<br>
<br>
<br>

::: {.text-bold}
Introduction to [GitHub]{.text-background-wisteria} and [Version Control]{.text-background-green-yellow}
:::

:::: {.r-hstack}
::: {data-id="box1" style="background: #fcf2dcff; width: 120px; height: 15px; margin: 0px;"}
:::

::: {data-id="box2" style="background: #c49cf5ff; width: 550px; height: 15px; margin: 0px;"}
:::

::: {data-id="box3" style="background: #040303ff; width: 120px; height: 15px; margin: 0px;"}
:::

::: {data-id="box3" style="background: #c2ff76ff; width: 120px; height: 15px; margin: 0px;"}
:::
::::

[27.3.2025]{.text-color-wisteria}

:::

::: {.column width="30%"}

```{r}
#| label: head_photo2
include_local_figure("About/head_photo.png")
```
:::

::::

<br>

:::: {.columns}

::: {.column width="15%"}

```{r}
#| label: personal_sticker
include_local_figure("About/personal_sticker.png")
```
:::

::: {.column width="55%"}
* 📧 Ondrej.mottl(at)gmail.com
* 🦋 [ondrejmottl.bsky.social](https://bsky.app/profile/ondrejmottl.bsky.social)
* 🌐 [bit.ly/ondrej_mottl](https://bit.ly/ondrej_mottl)
:::

::: {.column width="15%"}
```{r}
#| label: GH_logo
include_local_figure("Logos/GH_logo.png")
```
:::

::: {.column width="15%"}
```{r}
#| label: figure2
include_local_figure("flyer_cut.png")
```
:::



::::